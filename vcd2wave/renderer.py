...
def _leaf_name(full_name):
    """Extract the leaf signal name from a hierarchical path."""
    return full_name.split(".")[-1] if "." in full_name else full_name


def _sort_and_prepare(signals, timescale_info):
    """Convert signals dict to sorted list with formatted transitions.

    Returns list of dicts: [{name, width, trans, raw}]
    where 'name' is the leaf display name, 'trans' has formatted values.
    """
    ps_per_unit = timescale_info["ps_per_unit"]
    sig_list = []

    for code, sig in signals.items():
        if code == "__warnings__":
            continue
        vals = sorted(sig["values"], key=lambda x: x[0])
        w = sig["width"]
        display_name = _leaf_name(sig["name"])
        trans = []
        raw_trans = []
        for t, v in vals:
            # Convert time from VCD ticks to picoseconds
            t_ps = t * ps_per_unit
            if w == 1:
                display_val = "1" if v in ("1",) else "0"
                trans.append((t_ps, display_val))
            else:
                binary_str = v
                try:
                    ...
                    hex_val = binary_str
                trans.append((t_ps, hex_val))
            raw_trans.append((t_ps, v))
        sig_list.append({
            "name": display_name,
            "full_name": sig["name"],
            "width": w,
...
def _select_signals_for_display(signals, sig_list):
    """Choose which signals to show: prefer top-level, fall back to all.

    'Top-level' = signals whose full name has exactly 1 dot (e.g. 'top.clk').
    If too few, show everything.
    """
    top_level = [s for s in sig_list if s["full_name"].count(".") <= 1]
    if len(top_level) >= 5:
        return top_level
    return sig_list


def gen_html(signals, max_time, timescale_info, title="Waveform"):
    """Generate a self-contained HTML page rendering the waveform.

    Args:
        signals: dict from parse_vcd().
        max_time: int, max timestamp in VCD ticks.
        timescale_info: dict from parse_vcd().
    ...
    # Prepare signal data
    all_sigs = _sort_and_prepare(signals, timescale_info)
    sig_data = _select_signals_for_display(signals, all_sigs)

    # Convert max_time to ps for JS
    max_time_ps = max_time * timescale_info["ps_per_unit"]
    timescale_unit = timescale_info["unit"]

    # Separate single-bit and bus for ordering
    single_bit = [s for s in sig_data if s["width"] == 1]
    bus_signals = [s for s in sig_data if s["width"] > 1]
    ordered = single_bit + bus_signals

    sig_json = json.dumps(ordered)
    num_sigs = len(ordered)

    JS_CODE = r"""
let sigData = SIG_DATA_PLACEHOLDER;
let maxTime = MAX_TIME_PLACEHOLDER;
let timescaleUnit = TIMESCALE_UNIT_PLACEHOLDER;
...
// Parse the timescale unit to get ps per unit
function parseTimescaleUnit(unitStr) {
  var m = unitStr.match(/(\d+)(ps|ns|us|ms|s)/);
  if (!m) return 1;
  var num = parseInt(m[1]);
  var u = m[2];
  var scales = {ps:1, ns:1000, us:1000000, ms:1000000000, s:1000000000000};
  return num * (scales[u] || 1);
}
var PS_PER_UNIT = parseTimescaleUnit(timescaleUnit);
...
function fmtTime(t) {
  ...
  if (t < 1000000000) return (t/1000000).toFixed(t%1000000?2:0) + '\u00b5s';
  if (t < 1000000000000) return (t/1000000000).toFixed(2) + 'ms';
  return (t/1000000000000).toFixed(3) + 's';
}

function rawTs(t) {
  return t / PS_PER_UNIT;
}
...
function draw() {
  ...
  // Compute grid step in ps
  var gridStepPs = 1;
  if (maxTime > 1000) gridStepPs = 10;
  if (maxTime > 10000) gridStepPs = 100;
  if (maxTime > 100000) gridStepPs = 1000;
  if (maxTime > 1000000) gridStepPs = 10000;
  if (maxTime > 10000000) gridStepPs = 100000;
  if (maxTime > 100000000) gridStepPs = 1000000;
  if (maxTime > 1000000000) gridStepPs = 10000000;

  for (var t = 0; t <= maxTime; t += gridStepPs) {
    var x = t * px + LABEL_W;
    html += '<line x1=\"' + x + '\" y1=\"26\" x2=\"' + x + '\" y2=\"' + (showCompare?totalH-20:totalH) + '\" stroke=\"' + gridC + '\" stroke-width=\"1\"/>';
    var show = (maxTime/gridStepPs < 40) || (t % (gridStepPs*5) === 0);
    if (show) {
  ...
  function drawSignals(data, offset, suffix, colorShift) {
    for (var i = 0; i < data.length; i++) {
      ...
      var dn = sig.name;
      if (suffix) dn += ' ' + suffix;
      html += '<text x=\"8\" y=\"' + (y0+ROW_H/2+4) + '\" font-size=\"11\" font-weight=\"600\" fill=\"' + lc + '\">' + dn.replace(/&/g,'&amp;').replace(/</g,'&lt;') + '</text>';
      if (suffix) {
...
function renderAnnPanel() {
  ...
  for (var i=0; i<annotations.length; i++) {
    var a = annotations[i];
    h += '<tr><td class=\"ann-time\">' + fmtTime(a.time) + '</td><td class=\"ann-text\">' + a.label.replace(/</g,'&lt;') + '</td><td class=\"ann-del\"><button onclick=\"delAnn(' + i + ')\" class=\"del-btn\">\u2715</button></td></tr>';
  }
  h += '</table>';
  panel.innerHTML = h;
}
...
// Minimal VCD parser in JS (for client-side load/compare)
  ...
  for (var i = 0; i < lines.length; i++) {
    ...
    if (line.startsWith('$scope')) {
      ...
      if (parts.length >= 5) {
        ...
        var full = scopeStack.slice(1).concat([name]).join('.');
...
"""
    ...
    JS_CODE = (JS_CODE
        .replace("SIG_DATA_PLACEHOLDER", sig_json)
        .replace("MAX_TIME_PLACEHOLDER", str(max_time_ps))
        .replace("TIMESCALE_UNIT_PLACEHOLDER", json.dumps(timescale_unit))
        .replace("ROW_H_PLACEHOLDER", str(ROW_H))
        .replace("LABEL_W_PLACEHOLDER", str(LABEL_W))
    )
...
<div class="toolbar" id="toolbar" style="display:{'none' if num_sigs == 0 else 'flex'}">
  ...
  <span class="info">{num_sigs} signals | max {max_time_ps} ps</span>
